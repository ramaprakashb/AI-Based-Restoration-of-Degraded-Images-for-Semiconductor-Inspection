...
The feature representation passes through **8 such residual blocks**.
...
No unsupported PSNR, SSIM, or LPIPS values are reported for the blind test because matching ground-truth images are unavailable.

---

# Visual Quality Refinement

The restoration model is designed to address several image-quality issues.
...
Uses sharpness-aware learning to reduce excessive smoothing and preserve fine details.

### Contrast and Brightness

The quantitative validation metrics are evaluated without post-processing.

Optional contrast or sharpness adjustments used for blind-test visualization are treated separately from the reported quantitative model metrics.
...
- **Structure-Aware Restoration** — Uses structural and edge-aware objectives to preserve important image information.
...
The approach is designed to improve the visibility of fine semiconductor structures while maintaining a compact architecture suitable for further optimization and deployment in semiconductor inspection workflows.
